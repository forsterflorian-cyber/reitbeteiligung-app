...
const guestItems = [
  ...
  { href: "/signup" as Route, label: "Konto" },
...
const ownerItems = [
  { href: "/dashboard" as Route, label: "Uebersicht" },
  { href: "/owner/horses" as Route, label: "Neu" },
  { href: "/owner/pferde-verwalten" as Route, label: "Verwalten" },
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-stone-200 bg-white md:hidden">
      <div className="mx-auto grid max-w-md grid-cols-5 gap-1 px-2 pb-[calc(env(safe-area-inset-bottom)+0.4rem)] pt-2 sm:max-w-lg">
        {items.map((item, index) => {
          ...
          return (
            <Link
              className={`inline-flex min-h-[52px] items-center justify-center rounded-xl px-2 text-center text-[11px] font-semibold leading-tight ${
                active ? "bg-forest text-white" : "bg-stone-100 text-stone-600"
              }`}
              href={item.href}
              key={key}
            >
