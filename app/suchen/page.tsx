  ...
  return (
    <div className="space-y-5">
      <div className="space-y-2">
        ...
        <p className="text-sm text-stone-600 sm:text-base">Hier findest du freigeschaltete Eintraege und kannst direkt einen Probetermin vorbereiten.</p>
      </div>
      {!user ? (
        <Notice text="Melde dich an, damit du freigeschaltete Reitbeteiligungen sehen und einen Probetermin anfragen kannst." />
      ) : null}
      <div className="space-y-3">
        ...
        {horses.map((horse) => (
          <div className="rounded-3xl border border-stone-200 bg-white p-5 shadow-soft" key={horse.id}>
            <div className="space-y-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-clay">Reitbeteiligung</p>
              ...
              <Link
                ...
                href={profile ? "/anfragen" : "/login"}
              >
                {profile ? "Termin anfragen" : "Zur Anmeldung"}
              </Link>
            </div>
          </div>
        ))}
