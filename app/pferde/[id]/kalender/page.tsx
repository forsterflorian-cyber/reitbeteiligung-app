...
export default async function PferdKalenderPage({ params, searchParams }: PferdKalenderPageProps) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <Link className={buttonVariants("ghost", "min-h-0 justify-start px-0 py-0 text-sm font-semibold text-emerald-800 hover:bg-transparent")} href={detailHref}>
        Zurueck zum Pferdeprofil
      </Link>

      <PageHeader
        subtitle={`Kalender fuer ${horse.title}. Freie Fenster, belegte Zeiten und Terminanfragen liegen in einer Ansicht.`}
        title="Kalender"
      />
      ...
      <SectionCard subtitle="Schneller Ueberblick ueber freie Zeiten, belegte Bereiche und offene Anfragen." title="Naechste 14 Tage">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-7">
            {dayOverview.map((day) => (
              <Card className={day.isToday ? "border-emerald-300 bg-emerald-50 p-3" : "p-3"} key={toDayKey(day.date)}>
          ...
          {isOwner ? (
            <>
              ...
              <SectionCard subtitle="Nimm angefragte Termine an oder lehne sie ab." title="Offene Terminanfragen">
                <div className="space-y-4">
                  {requestedOwnerBookingItems.length === 0 ? (
                    <EmptyState
                      description="Fuer dieses Pferd liegen derzeit keine offenen Terminanfragen vor."
              ...
              <div className="grid gap-5 lg:grid-cols-2">
                <SectionCard
                  subtitle="Beim Entfernen werden offene Terminanfragen in diesem Fenster automatisch geloescht."
                  title="Eigene Verfuegbarkeitsfenster"
                >
                  <div className="space-y-3">
                    {rules.length === 0 ? (
                      <EmptyState
                        description="Lege zuerst ein Verfuegbarkeitsfenster an, damit Reiter Termine anfragen koennen."
                        title="Noch keine Verfuegbarkeiten"
                      />
                    ) : (
                      rules.map((rule) => (
                        <Card className="p-4" key={rule.id}>
                          ...
                          <form action={deleteAvailabilityRuleAction} className="mt-3">
                            ...
                            <ConfirmSubmitButton
                              className="inline-flex min-h-[44px] w-full items-center justify-center rounded-xl border border-stone-300 bg-white px-4 py-3 text-sm font-semibold text-stone-900 hover:border-emerald-700 hover:text-emerald-800 disabled:cursor-not-allowed disabled:opacity-70"
                ...
                <SectionCard subtitle="Nur diese selbst gesetzten Sperren kannst du wieder entfernen." title="Eigene Sperren">
                  <div className="space-y-3">
                    {ownerBlocks.length === 0 ? (
                      <EmptyState
                        description="Nutze Sperren fuer Ausnahmen, wenn das Pferd kurzfristig nicht verfuegbar ist."
